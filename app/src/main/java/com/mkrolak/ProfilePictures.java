...
public class ProfilePictures {
    ...
    public static int getPictureDrawableFromUri(Uri uri){
        return  getPictureDrawableFromString(uri.toString());
    }

    public static int getPictureDrawableFromString(String s){
        return getPictureDrawableFromInt(Integer.parseInt(s.split("user")[1].replace(".png","").trim()));
    }

    public static int getPictureDrawableFromInt(int i){
        return  LIST_OF_DRAWABLES[i];
    }
}
